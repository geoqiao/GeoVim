...
-- 目标：简洁、现代、对新手极度友好。
-- 要求：Neovim 版本 >= 0.12（使用原生 LSP 新 API）

-- 硬检查：Neovim 版本必须 >= 0.12
if vim.fn.has("nvim-0.12") ~= 1 then
    vim.notify("GeoVim requires Neovim >= 0.12. Please upgrade your Neovim.", vim.log.levels.ERROR)
    return
end
    ...
    performance = {
        rtp = {
            disabled_plugins = {
                ...
                "tutor",
                "rplugin",
                -- "syntax",          -- 保留：作为 treesitter fallback
                -- "synmenu",         -- 保留
                "optwin",
                -- "compiler",        -- 保留：:compiler 预设
                "bugreport",
                -- "ftplugin",        -- 保留：文件类型默认设置至关重要
            },
        },
    },
})
